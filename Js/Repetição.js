...
function validar(){
    ...
    if (posicao2.right === posicao1.left && posicao1.right === posicao3.left){
        $('#quadrado').animate({left:'100px'},{duration:800})
        $('#quadrado').animate({top:"40px"},{duration:800})
        $('#quadrado').animate({left:"-6px"},{duration:800})
        $('#quadrado').animate({top:"-6px"},{duration:800})
        setTimeout(()=>{
            var codigoDestacado = `
                for (let i = 0; i < 4; i++) {
                    moveForward(100);
